...
vi.mock('../cardsData', () => ({
  cards: [
    { id: 1, name: 'Guard', strength: 1, effect: 'Guess another player\'s card.' },
    { id: 2, name: 'Priest', strength: 2, effect: 'Look at another player\'s card.' },
    { id: 3, name: 'Baron', strength: 3, effect: 'Compare cards with another player.' },
    { id: 4, name: 'Handmaid', strength: 4, effect: 'Protect yourself until your next turn.' },
    { id: 5, name: 'Prince', strength: 5, effect: 'Force a player to discard and draw.' },
  ]
}));
...
describe('Baron Card Effects', () => {
  ...
  describe('applyBaronEffect', () => {
    it('should eliminate target when attacker has higher strength', async () => {
      ...
      expect(result.attackerMessage).toContain('Your Baron\'s duel is victorious');
      expect(result.targetMessage).toContain('You are eliminated from the round');
      
      // Verify Firebase update to eliminate target
      expect(update).toHaveBeenCalledWith(
        undefined, // ref result
        { isOut: true }
      );
    });
    ...
    it('should eliminate attacker when target has higher strength', async () => {
      ...
      expect(result.attackerMessage).toContain('Your Baron\'s duel ends in defeat');
    ...
    it('should generate medieval-themed messages for all scenarios', async () => {
      ...
      expect(result.publicMessage).toMatch(/⚖️💥.*Baron.*duel.*eliminated.*⚔️👑/);
    });
    ...
    it('should handle maximum strength difference', async () => {
      ...
      expect(result.publicMessage).toContain('Guard (1) falls to superior strength');
    });
    ...
    it('should handle minimum strength difference', async () => {
      ...
      expect(result.eliminatedPlayer).toBe('alice');
      expect(result.winner).toBe('bob');
      expect(Math.abs(result.attackerCard.strength - result.targetCard.strength)).toBe(1);
    });
  });
});
